...
- 連測試都會被迫圍繞 workaround 來寫，而不是直接驗證框架行為
...
簡單講，新的方向是：exception handler 可以把要送去 DLQ 的 records 回交給框架，讓框架自己送。這個能力是加在 Kafka Streams 內建的 deserialization / processing / production exception handling 流程上；本文的 `after/` 範例，則刻意只示範最常見、也最麻煩的 deserialization error。
...
| --- | --- | --- |
| Processing error | 可以自己攔，但通常得自己把錯誤導去 DLQ | 可改走 4.2.0 新增的 processing exception handler 路徑 |
